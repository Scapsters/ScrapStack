...
export default function StackSearch() {
	...
	const searchedStacks = useMemo(
		() =>
			(stacks && search && new FuzzySearch(stacks, ['twitterHandle', '_id']).search(search)) ||
			(stacks && stacks.sort((a, b) => a.postCount - b.postCount)) ||
			[],
		[search, stacks]
	)
				...
				<div className="flex flex-col items-center mt-5 w-175 px-5">
					...
					{isLoading ? (
						...
						searchedStacks.map((stack) => (
							<Link
								to={`/stacks/${stack.twitterHandle}`}
								className="flex mt-2 justify-between items-center p-2 sm:px-15 border-1 border-dark/20 rounded-md w-3/4 h-12 sm:h-20 text-dark hover:bg-cyan-light/50 hover:border-2"
							>
