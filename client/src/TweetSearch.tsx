...
export function TweetSearch(props: { tweetFilter: Partial<TweetSchema>; isLoading: boolean }) {
	...
	const { getValues, register } = useForm<typeof defaultSearchValues>({
		defaultValues: { ...defaultSearchValues, ...props.tweetFilter },
	})
	...
	return (
		<SideBar isOpen={isSettingsOpen} setIsOpen={setIsSettingsOpen}>
			...
			>
				<div
					className={`absolute flex items-center justify-center w-full h-full rounded-lg ${props.isLoading ? 'bg-black/20 z-40' : 'invisible -z-10'}`}
				>
