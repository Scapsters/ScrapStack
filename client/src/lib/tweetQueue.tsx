...
import { NewTweetBatch, TweetBatch } from '../Tweet'
import throttle from 'lodash/throttle'
import type { defaultSearchValues } from '@/formConsts'
...
export function useNewTweetQueue(
	getFirstTweet: (() => TweetQuery) | null,
	getNextTweet: (batchIndex: number) => TweetQuery,
	stackRef: RefObject<HTMLDivElement | null>
) {
	const [tweetBatches, setTweetBatches] = useState<Parameters<typeof NewTweetBatch>['0']['batch'][]>([])
	const batchIndexRef = useRef(0)

	const addBatchToBatches = useCallback((batch: Awaited<ReturnType<typeof getNextTweet>>) => {
		const batchWithBlobs = batch.map(data => ({
			data,
			mediaUrlBlobs: getMediaBlobs(data),
		})) satisfies TweetWithURLs[]
		setTweetBatches(previous => [...previous, batchWithBlobs])
	}, [])

	useEffect(() => {
		if (tweetBatches.length === 0) getFirstTweet?.().then(addBatchToBatches)
	}, [addBatchToBatches, getFirstTweet, tweetBatches.length])
	...
	}, [addBatchToBatches, getNextTweet, stackRef])
...
export function useTweetQueue(
	...
	const view = useCallback(
		(batchIndex: number) => (key: string, isRepeat: boolean) => {
			setCache(prev => ({
				...prev,
				[queryName]: {
					...prev[queryName],
	...
	const makeTweetBatches = useCallback(
		(promise: ReturnType<typeof extract>, batchIndex: number) => {
	...
	useEffect(() => {
		const fetchTweets = async () => {
			await Promise.resolve()
			if (Date.now() - lastFetch.current > 100) {
				// throttle mainly for page load
				const stack = stackRef.current
				if (stack) {
					const distanceToBottom = stack.getBoundingClientRect().bottom
	...
	useEffect(() => setDoTweetsExist(true), [queryName, setDoTweetsExist])

	return [virtualizeBatches(cache[queryName], scrollTop), isLoading, setIsLoading] as const
...
function virtualizeBatches(cache?: Cache[string], scrollTop?: number): JSX.Element[] {
	...
	const visibleBatches = [
		...cache.heights
			.slice(0, startIndex)
			.map((height, index) => <div style={{ height: height + 'px' }} key={'spacer top' + index}></div>),
		...cache.batches
			.slice(startIndex, endIndex)
			.map((batch, index) => spreadIntoTweetBatch({ ...batch, minHeight: cache.heights[startIndex + index] })),
		...cache.heights
			.slice(endIndex)
			.map((height, index) => <div style={{ height: height + 'px' }} key={'spacer bottom' + index}></div>),
	]
	const unloadedBatchesInjection = visibleBatches.map((element, index) =>
		cache.unloadedBatchIndices.includes(index) ? spreadIntoTweetBatch(cache.batches[index]) : element
	)
